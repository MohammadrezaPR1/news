...
import { FiVideo, FiSave, FiX, FiInfo, FiUpload, FiPlay, FiEdit2 } from "react-icons/fi";
...
const EditVideo = () => {
    const { errorVideo, editVideo } = useContext(AdminContext);
        ...
        <Dashboard>
            <div dir="rtl" className="max-w-4xl mx-auto px-4 py-10">
                <div className="flex items-center justify-between mb-10">
                    <div>
                        <h2 className="text-3xl font-black text-slate-900 mb-2 border-b-4 border-indigo-500 pb-2 w-fit">ویرایش ویدیو</h2>
                        <p className="text-slate-500 font-bold text-sm">به‌روزرسانی اطلاعات محتوای ویدیویی</p>
                    </div>
                    <button
                ...
                <form onSubmit={formik.handleSubmit} className="space-y-8">
                    <div className="bg-white rounded-[40px] p-8 md:p-12 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-32 h-32 bg-amber-50/50 rounded-full -ml-16 -mt-16 blur-3xl" />
                        ...
                        <div className="relative space-y-8">
                            ...
                            <div className="space-y-4 pt-4">
                                <div className="flex items-center gap-2 mb-2">
                                    <div className="w-1.5 h-6 bg-indigo-500 rounded-full" />
                                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest">تغییر فایل ویدیو (اختیاری)</label>
                                </div>
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div className="relative">
                                        ...
                                        <div className="h-[250px] border-2 border-dashed border-slate-200 rounded-[32px] bg-slate-50 flex flex-col items-center justify-center p-8 text-center group hover:border-indigo-300 hover:bg-indigo-50/30 transition-all">
                                            <div className="w-16 h-16 bg-white rounded-2xl shadow-sm border border-slate-100 flex items-center justify-center text-amber-500 mb-4 group-hover:scale-110 transition-transform">
                                                <FiUpload size={28} />
                                            </div>
                                            <p className="text-sm font-black text-slate-700 mb-1">تغییر ویدیو فعلی</p>
                                            <p className="text-[10px] font-bold text-slate-400">فایل جدید را اینجا بکشید</p>
                                        </div>
                                    </div>

                                    {/* Preview */}
                                    <div className="h-[250px] rounded-[32px] bg-slate-900 overflow-hidden flex items-center justify-center relative shadow-inner">
                                        {previewUrl ? (
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-[10px] font-black text-slate-700 uppercase tracking-tight">عدم وجود ویدیو</p>
                                            </div>
                                        )}
                                        {previewUrl && (
                    ...
                    <div className="flex items-center justify-end gap-4">
                        <button
                            type="button"
                            onClick={() => window.history.back()}
                            className="px-10 py-4 rounded-[20px] font-black text-sm text-slate-400 hover:text-slate-600 transition-colors"
                        >
                            لغو تغییرات
                        </button>
                        <button
                            type="submit"
                            className="bg-amber-500 hover:bg-amber-600 text-white px-12 py-4 rounded-[20px] font-black text-sm shadow-xl shadow-amber-100 transition-all hover:-translate-y-1 flex items-center gap-2"
                        >
                            <FiEdit2 size={20} />
                            <span>ثبت تغییرات نهایی</span>
                        </button>
                    </div>
                </form>
            </div>
